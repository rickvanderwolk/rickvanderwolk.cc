...
function buildProjectsHtml(array $projects): string
{
    $projectsHtml = '<div id="projects">';

    foreach ($projects as $index => $project) {
        $rowClass = $index % 2 === 0 ? 'row' : 'row row-reverse';
        if ($index === 0) {
        ...
        if (!empty($project['image'])) {
            $imageLink = !empty($project['liveUrl']) ? $project['liveUrl'] : (!empty($project['codeUrl']) ? $project['codeUrl'] : '');
            if ($imageLink) {
                $imageHtml = "<div class=\"image-col\" style=\"border: 3px solid {$color};\"><a href=\"{$imageLink}\" target=\"_blank\" rel=\"noopener noreferrer\"><img src=\"{$project['image']}\" loading=\"lazy\"></a></div>";
            } else {
                $imageHtml = "<div class=\"image-col\" style=\"border: 3px solid {$color};\"><img src=\"{$project['image']}\" loading=\"lazy\"></div>";
            }
        } else {
            $imageHtml = "<div class=\"image-col\"></div>";
        }

        $projectsHtml .= "<div class=\"{$rowClass}\">
            {$imageHtml}
